...
class ThirdPage : Fragment(), OnAttendEventListener {
    ...
    private val eventListAdapter: GenericAdapter<EventModelRealm> by lazy {
        GenericAdapter<EventModelRealm>(
            EventListAdapter(
                requireActivity(), null, this
            )
        )
    }

    override fun attendEvent(time: Long) {
        realmControllerEvent.updateDataEvent(time, true){
